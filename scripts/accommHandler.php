...
//{--------------------------------------------- Main Switch ---------------------------------------------
	...
	switch( $status )
	{
		...
		case 'checkAvail':
			if ( $_POST['rooms'] == "" )
			{
				$statusMessage = "Oops!  Your selection didn't go throughi for some reason.  Please re-enter your room qty.";
				break;
			}
...
//{--------------------------------------------- Helper Functions ---------------------------------------------
	...
	function deleteReservations( $whereClause )
	{
		...
		{
			$log->debug( "No reservations to delete." );
			return "";
		}
		...
		foreach ( $roomTotals as $roomType => $roomCount )
		{
			...
			$log->debug( "Updated $roomType remain from {$row[0]} to $newRoomCount" );
		}			
		...
		foreach ( $guestReimbursements as $curGuestID => $guestResDue )
		{
			...
			$log->debug( "Updated guest #$curGuestID total_due from \${$row[0]} to \$$newTotalDue, num_rooms from {$row[1]} to $newNumRooms" );
		}
