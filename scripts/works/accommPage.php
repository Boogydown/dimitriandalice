...
<script type="text/javascript">
	...
	function guestSelected( guestID )
	{
		...
		else 
		{
			...
			if ( guestID != " " )
			{
				...
				$.ajax( { data:{ status:"checkForRes", guestID:guestID}, success:pendingRes } );
			} //guest!=" "
			else
				resetForm();
		}
	...
	function resetForm()
	{
		busyIndic.stop();
		$(":text").val("");
		$.ajax( { data:{ status:"getRoomRemains" }, success:reloadRemains } );
		updateTotal( );		
		//guestSelected( " " );
		$("#hideableSection").hide("normal");
	}	
	 ...
	 */
	var spinLockArgs = { spinning:false };
	function spinLockPost( data, callback, xhr )
	{
		// if global is null then this is original call
		if ( !spinLockArgs.spinning )
		{
			spinLockArgs = { spinning:true, data:data, success:callback };
		...
		else
		{
			...
			{
				if ( data.status.substr( 0, 9 ) == "Locked by" )
					busyIndic.status = "\n" + data.status + ". Please wait...";
				window.setTimeout( function(){$.ajax( { data: spinLockArgs.data, success: spinLockPost } );}, 5000 );
				return;
			}
