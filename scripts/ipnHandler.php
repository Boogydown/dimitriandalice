...
if (strcmp ($ppResponseAr["httpResponse"], "VERIFIED") == 0) 
{
	...
	switch( $_POST['payment_status'] )
	{
		...
		case 'Processed':
			...
			// Secure table lock
			$log->debug("Securing table lock: " . Utils::Spinlock( TABLE_LOCKS, -1, FALSE, TRUE, $guestName ) );
			...
			if ( $result == FALSE )
			{
				...
				$log->debug( "Updated guest record for guestID #$guestID: paid $grossPmnt for total of {$row['total_paid']}" );
			...
			foreach ( $resIDs as $resID )
			{
				...
				if ( $resRoomInfo = mysql_fetch_array( $result ) )
				{
					...
					if ( $resRoomInfo['num_1nts'] > 0 ) $oneNighter = TRUE;
					if ( $resRoomInfo['num_2nts'] > 0 ) $twoNighter = TRUE;
			...
			$mailMessage .="\n\tAmount paid: \$$grossPmnt\n\n" . 
						   ...
						   ( $oneNighter ? "\tYour 1-night stay is for Saturday night.  If you would like to stay Friday night, instead, please email Dimitri@DimitriAndAlice.com.\n" : "" ) .
						   ( $twoNighter ? "\tYour 2-night stay is for Friday and Saturday nights.\n" : "" ) .
						   "\tCheck-in starts at 10am, check-out is by 5pm.\n\n" .
	...
	$log->error( "IPN Post Response: {$ppResponseAr["httpResponse"]}");

if ( $log->hasErrored() )
	mail( "boogydown@gmail.com", "Paypal error", implode("\n", $postFieldsAr) );
